...
%% open figure with specified size
h=figure('Units', 'pixels', 'Position', [100 100 500 375]);
hold on;

%% first, make a very basic and ugly plot
hFit   = plot(xfit  , yfit   );
hE     = errorbar(xdata_m, ydata_m, ydata_s);
hData  = plot(xVdata, yVdata );
hModel = plot(xmodel, ymodel );
hCI(1) = plot(xmodel, ymodelL);
hCI(2) = plot(xmodel, ymodelU);
...
print -depsc2 finalPlot1Orig.eps
